...
var RealexHpp = (function () {
	...
	var internal = {
		...
		createIFrame: function (overlayElement, token) {
			...
			let iframeHeight = Math.max(562, window.innerHeight * 0.9);
			console.log("iframeHeight = " + iframeHeight);
			iFrame.setAttribute("height", iframeHeight + "px");
			iFrame.setAttribute("frameBorder", "0");
			let iframeWidth = Math.max(360, Math.floor(screen.width*0.7));
			...
			if (isMobileIFrame) {
				...
				iFrame.style.top =  Math.floor((window.innerHeight - iframeHeight)/2.0) + "px"; // "40px";
				iFrame.style.left = "50%";
				iFrame.style.marginLeft = "-" + Math.floor(iframeWidth/2.0) + "px";
			}
